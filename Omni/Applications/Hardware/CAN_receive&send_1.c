...
board_communication_data board_communication_data_0;
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  if (rx_header.StdId == 0x307)
  {
	...
	//测试数据	
		if(rx_header.StdId ==0x094)
		{
			get_measure(&board_communication_data_0,rx_data);
			return;
		}	
...
void get_measure(board_communication_data *ptr, uint8_t data[])
{
//  (ptr)->test0 = data[0];
//  (ptr)->test1 = data[1];
//  (ptr)->test2 = data[2];
...
//  (ptr)->test6 = data[6];
//  (ptr)->test7 = data[7];	
	memcpy(&(ptr)->test0,&data[0],4);
	memcpy(&(ptr)->test1,&data[4],4);
}


void CAN1_send_board_data() 
{
  uint8_t can_send_data[8];
  static CAN_TxHeaderTypeDef tx_message;
  uint32_t send_mail_box;
  ...
  tx_message.DLC = 0x08;
	...
	memcpy(&can_send_data[0],&IMU_data.AHRS.pitch,4);
	memcpy(&can_send_data[4],&IMU_data.AHRS.yaw,4);
//  can_send_data[0] = 45;
//  can_send_data[1] = 87;
