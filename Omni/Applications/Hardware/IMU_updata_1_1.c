...
void IMU_init()
{
	...
	pid_set(&IMU_tempure_pid, 1500, 0.01, 0, 5000-1, 300); // PID设置300 CCR最大4999
...
{
//	// 计算总角度
//	IMU_data.AHRS.yaw_angle_cnt = IMU_data.AHRS.yaw_rad_cnt * 57.3f;
}
...
//	IMU_data.gyro[2] -= IMU_data.calibration[2];
			IMU_data.gyro[0] -= 0.006348;
			IMU_data.gyro[1] -= 0.004157;
			IMU_data.gyro[2] -= 0.000740;
...
void bsx_update()
{
	...
	IMU_data.BSX.pitch = - bsxlite_fusion_out.orientation.roll;//为了与AHRS解算方向一致
	IMU_data.BSX.yaw = bsxlite_fusion_out.orientation.yaw;
	IMU_data.BSX.roll =  - bsxlite_fusion_out.orientation.pitch;//为了与AHRS解算方向一致
}
// end of file
