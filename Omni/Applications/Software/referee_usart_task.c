...
static void referee_unpack_image_fifo_data(void);
fifo_s_t referee_fifo;
fifo_s_t referee_image_fifo;
...
void referee_usart_task(void)
{
  ...
  referee_unpack_image_fifo_data();
}
...
void referee_unpack_image_fifo_data(void)
{
  uint8_t byte = 0;
  uint8_t sof = HEADER_SOF;
  unpack_data_t *p_obj = &referee_image_unpack_obj;

  while (fifo_s_used(&referee_image_fifo))
  {
    byte = fifo_s_get(&referee_image_fifo);
    switch (p_obj->unpack_step)
    {
    case STEP_HEADER_SOF:
    {
