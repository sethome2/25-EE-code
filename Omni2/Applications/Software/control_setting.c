...
/******************************PC操作逻辑**************************************************/
	else if (Global.input.ctl==PC) // keyborad&mouuse_input_begin
	{
      ...
      if(Global.input.fly_status!=1)//飞坡模式中不记录pitch轴数据
			Global.input.pitch =MOUSE_Y_MOVE_SPEED/3000.0f;	//			Global.input.yaw =AutoAim_Algorithm(&mouse_x_kf_fliter,MOUSE_X_MOVE_SPEED/7000.0f);
//			Global.input.pitch =AutoAim_Algorithm(&mouse_y_kf_fliter,MOUSE_Y_MOVE_SPEED/6000.0f);
		}
		/*************底盘模式切换****************/
		//开启小陀螺模式
		if (IF_KEY_PRESSED_Q)
		...
		{
			Global.cap = FULL;
			Global.input.fly_speed_up_status =1;
		}
		else
		{
			Global.cap = STOP;
			Global.input.fly_speed_up_status =0;
		}
 ...
 //         Global.input.y+=fly_speed_up_ex(1,0.001f,0.0025f,Global.input.y);
           Global.input.y+=0.04;
		...
		else if (IF_KEY_PRESSED_S)
		{
			if (Global.cap == FULL)
			{
//				if (Global.input.y > -2.0f * KS)
//					Global.input.y -= 0.08f;
...
//		}			
		...
		{
		//	if(Get_sys_time_ms()-Time_delay_resetcan>350){
				HAL_CAN_DeInit(&hcan1);
				osDelay(10);
				MX_CAN1_Init();
				can_filter_init();
			//	Time_delay_resetcan=Get_sys_time_ms();
			//}
		}
