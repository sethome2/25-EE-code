...
void shoot_pid_cal(void)//现在逻辑太乱了，以后建议重新整理一下（我也这么觉得，学弟学妹加油！！！）
{
	static enum trigger_status_e last_status = LOCATIONS;
  float set;
  if(Global.input.shoot_num==1&&Global.input.shoot_fire&&Global.input.shooter_status)
    set=6000.0f;
  else if(Global.input.shoot_num==0&&Global.input.shoot_fire&&Global.input.shooter_status)
    set=3000.0f;
  else set=0.0f;
	//摩擦轮设定
	decode_as_3508(SHOOT_MOTOR1);
  ...
  if(Global.input.shoot_fire)
	...
	else if(Global.input.vision_status == 1)
	{
			...
			shoot.set_trigger_speed=Global.input.shoot_RC*10;
	//速度环
...
void RC_trigger_anti_kill_and_set_speed(float set)//给遥控器任务单独用的，用于裁判系统剑录时防止卡弹
{
